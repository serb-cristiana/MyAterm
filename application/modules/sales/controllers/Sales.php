...
class Sales extends Admin_Controller
{
    ...
    if($products) {
      ...
      echo '<option value="">Înregistrările nu au fost găsite!</option>';
    }
    }

    public function add()
    {
        $this->layout->set_title('Adăugare ieșiri');
        $this->breadcrumbs->admin_push('Dashboard', 'dashboard');
        $this->breadcrumbs->admin_push('Listă ieșiri', 'sales');
        $this->breadcrumbs->admin_push('Adăugare ieșiri', 'sales/add');
        $data = array(
            'button' => 'Add',
    ...
    { //var_dump($this->input->post());die;
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          if($result){
            $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a adăugat o ieșire la '.date("M d, Y H:i")));
            $this->session->set_flashdata(array('message'=>'Ieșirea a fost înregistrară cu succes','type'=>'success'));
          }else {
    ...
    public function delete()
    {
      ...
      if($result){
        $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a șters o ieșire la '.date("M d, Y H:i")));
        echo json_encode(array('message'=>'Ieșirea a fost ștearsă cu succes.','type'=>'success'));
      }else {
    ...
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('product_category', 'Categorie de produse', 'trim|required');
        $this->form_validation->set_rules('brand', 'Categorie de branduri', 'trim|required');
        $this->form_validation->set_rules('prod_id[]', 'Categorie de produse', 'trim|required');
