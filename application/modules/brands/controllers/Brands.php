...
class Brands extends Admin_Controller
{
    ...
    public function add()
    {
        $this->layout->set_title('Adăugare Brand');
        $this->layout->add_js('../public/pekeupload/js/pekeUpload.js');
        $this->breadcrumbs->admin_push('Dashboard', 'dashboard');
        $this->breadcrumbs->admin_push('Listă Brandurilor', 'brands');
        $this->breadcrumbs->admin_push('Adăugare Brand', 'brands/add');
        $data = array(
            'button' => 'Add',
    ...
    public function add_brands()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          if($result){
            $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a adăugat un brand la '.date("M d, Y H:i")));
            $this->session->set_flashdata(array('message'=>'Brandul a fost adăugat cu succes.','type'=>'success'));
          }else {
    ...
    {   $this->layout->add_js('../public/pekeupload/js/pekeUpload.js');
        if(check_post()){
          ...
          if($result){
            $this->session->set_flashdata(array('message'=>'Brandul a fost actualizat cu succes.','type'=>'success'));
          }else {
            $this->session->set_flashdata(array('message'=>'Ceva nu a mers bine. Încearcați din nou!','type'=>'warning'));
          }
          redirectToAdmin('brands');
    ...
    public function delete()
    {
      ...
      if($result){
        $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a șters un brand la '.date("M d, Y H:i")));
        echo json_encode(array('message'=>'Brandul a fost șters cu succes.','type'=>'success'));
      }else {
