...
class Products extends Admin_Controller
{
    ...
    public function get_product_inventory(){
      ...
      if($result){
        echo json_encode(array('message'=>'Date despre stoc.','type'=>'success',"data"=>$result));
      }else {
    ...
    public function add()
    {
        $this->layout->set_title('Adăugare Produse');
        $this->layout->add_js('../public/pekeupload/js/pekeUpload.js');
        $this->breadcrumbs->admin_push('Dashboard', 'dashboard');
        $this->breadcrumbs->admin_push('Listă produse', 'products');
        $this->breadcrumbs->admin_push('Adăugare produse', 'products/add');
        $data = array(
            'button' => 'Add',
    ...
    public function add_products()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          if($result){
            $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a adăugat un produs la '.date("M d, Y H:i")));
            $this->session->set_flashdata(array('message'=>'Produsele au fost adăugate cu succes.','type'=>'success'));
          }else {
    ...
    public function edit_products($prod_id){
     ...
     $result=$this->products_model->edit($prod_id,$data_to_update);
      ...
      if($result){
        $this->session->set_flashdata(array('message'=>'Produsele au fost actualizate cu succes.','type'=>'success'));
      }else {
        $this->session->set_flashdata(array('message'=>'Ceva nu a mers bine. Încearcați din nou!','type'=>'warning'));
      }
      redirectToAdmin('products');
    ...
    public function delete()
    {
      ...
      if($result){
        $this->activity_model->add(array('login_id'=>$this->login_id,'activity'=>ucfirst($this->username).' a șters un produs la '.date("M d, Y H:i")));
        echo json_encode(array('message'=>'Produsul a fost șters cu succes.','type'=>'success'));
      }else {
        echo json_encode(array('message'=>'Ceva a mers prost încercați din nou.','type'=>'warning'));
      }
    }
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('product_category', 'Categorii de produse', 'trim|required');
        $this->form_validation->set_rules('brand', 'Categorii de branduri', 'trim|required');
        $this->form_validation->set_rules('attributes[]', 'Categorii de atribute', 'trim|required');
